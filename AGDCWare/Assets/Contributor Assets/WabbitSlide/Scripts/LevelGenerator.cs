...
namespace buckslice {
    ...
    public class LevelGenerator : MonoBehaviour {
        ...
        void Start() {
            bunny = GameObject.Find("Bunny").GetComponent<BunnyController>();
        ...
        private GameObject GenerateSlice() {
            ...
            if (setSpawn) { // dont spawn first item
                if (Random.value < 0.3f) {  // spawn carrot
                    spawn.x += (Random.value * 2f - 1f) * ((width / 2f) - displace);
                    BoostCarrot bc = ((GameObject)Instantiate(carrot, spawn, Quaternion.identity)).GetComponent<BoostCarrot>();
                    bc.bunny = bunny.transform;
                } else {    // spawn barrel
                    spawn.x += (Random.value * 2f - 1f) * ((width / 2f) - displace);
                    BoomBarrel bb = ((GameObject)Instantiate(barrel, spawn, Quaternion.Euler(0, Random.Range(-60, 60), 90))).GetComponent<BoomBarrel>();
                    bb.init(bunny);
                }
            }
            ...
            if (!setSpawn) {
                ...
                bunny.transform.position = new Vector3(v.x + p.x, 15f + p.y, 15f + p.z);
                bunny.rbody.velocity = new Vector3(0.0f, -1.0f, 1.0f) * 10.0f;
                setSpawn = true;
            }
        ...
        void Update() {
            ...
            GameObject slice = slices[0];   // check oldest slice
            if (slice.transform.position.z < bunny.transform.position.z - sliceLength * 3) {
                slices.Remove(slice);
